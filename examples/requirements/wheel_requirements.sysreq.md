...
The vehicle SHALL be configured with exactly [@wheel_count](/examples/vehicle_params.yaml?version=1#wheel_count) wheels as specified in the vehicle parameters.
...
- Physical wheel count MUST match parameter [@wheel_count](/examples/vehicle_params.yaml?version=1#wheel_count)
- Each wheel MUST have independent sensor monitoring
- Wheel failure detection MUST account for all 4 wheels

### Verification

Testing performed based on:
